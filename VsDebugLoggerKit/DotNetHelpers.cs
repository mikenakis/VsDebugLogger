...
public static class DotNetHelpers
{
	...
	public static Sys.DateTime UnixTimeStampToDateTime( double unixTimeStamp )
	{
		...
		var dtDateTime = new Sys.DateTime( 1970, 1, 1, 0, 0, 0, 0, Sys.DateTimeKind.Utc );
	...
	{
		var localAppDataFolder = DirectoryPath.FromAbsolutePath( Sys.Environment.GetFolderPath( Sys.Environment.SpecialFolder.LocalApplicationData ) );
	...
	public static bool JaggedIsNormalAssertion( Sys.Array jagged )
	{
		...
		int recurse( int dimension, Sys.Array jagged )
		{
			...
			if( dimension == rank - 1 )
				return length;
			var firstJaggedChild = (Sys.Array)NotNull( jagged.GetValue( 0 ) );
			int firstChildLength = recurse( dimension + 1, firstJaggedChild );
			for( int index = 1; index < length; index++ )
			{
				var anotherJaggedChild = (Sys.Array)NotNull( jagged.GetValue( index ) );
	...
	public static Sys.Array JaggedFromMultiDimensional( Sys.Array multiDimensional )
	{
		...
		Sys.Array recurse( int dimension )
		{
			...
			var jagged = Sys.Array.CreateInstance( jaggedArrayElementType, length );
	...
	public static Sys.Array MultiDimensionalFromJagged( Sys.Array jagged )
	{
		...
		var multiDimensional = Sys.Array.CreateInstance( elementType, GetJaggedLengths( jagged, rank ) );
		int[] indices = new int[multiDimensional.Rank];
		recurse( 0, jagged );
			...
			where K : notnull
	{
		...
		var dictionary = new Dictionary<K, V>( values.Count );
			...
			where E : struct, Sys.Enum
	{
		...
		var enumValue = (E)(object)intValue; //see https://stackoverflow.com/a/51025027/773113
