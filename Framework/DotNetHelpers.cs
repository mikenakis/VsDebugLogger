...
namespace Framework
{
    ...
    public static class DotNetHelpers
	{
		...
		private static string get_main_module_name()
		{
			...
			return mainModuleName.Substring( 0, mainModuleName.Length - exeExtension.Length );
		}
		...
		public static IReadOnlyDictionary<K, V> DictionaryFromValues<K, V>( IEnumerable<V> valuesEnumerable, Function<K, V> keyExtractor ) where K: notnull
		{
		...
		public static ISet<T> NewIdentitySet<T>() where T : class
		{
		...
		{
			System.Reflection.Assembly locatorAssembly;
			string fullResourceName;
			if( locatorType == null )
			{
				locatorAssembly = NotNull( System.Reflection.Assembly.GetEntryAssembly() );
